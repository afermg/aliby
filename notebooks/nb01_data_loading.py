...
@app.cell
def _(Path, get_data_path, mo):
    data_path = get_data_path(Path)
    mo.md(f"**Data directory:** `{data_path}`")
    return (data_path,)


@app.cell
def _(mo):
    mo.md("## 2. Dataset Discovery")
    return
...
@app.cell
def _(dataset_catalog, mo):
    catalog = dataset_catalog()
    mo.md(
        "### Available test datasets\n\n"
        + "\n".join(
            f"- **{d['name']}** — {d['description']} (`{d['capture_order']}`)"
            for d in catalog
        )
    )
    return (catalog,)
...
@app.cell
def _(data_path, ds_dropdown, load_dataset_dir, mo):
    ds_info = ds_dropdown.value
    dset, positions = load_dataset_dir(data_path, ds_info)
    mo.md(
    ...
    return dset, ds_info, positions


@app.cell
def _(mo):
    mo.md("## 3. Zarr Dataset Discovery")
    return


@app.cell
def _(data_path, ds_info, mo):
    from aliby.io.dataset import DatasetZarr, dispatch_dataset

    zarr_name = f"{ds_info['name']}.zarr"
    zarr_path = data_path / zarr_name
    ...
    return DatasetZarr, dset_zarr, dispatch_dataset, zarr_positions


@app.cell
def _(mo):
    mo.md("## 4. Image Loading")
    return
...
@app.cell
def _(ds_info, load_image_from_position, mo, np, positions):
    pos = positions[0]
    ...
    return data, dims, img, pos


@app.cell
def _(mo):
    mo.md("## 5. Image Visualization")
    return
