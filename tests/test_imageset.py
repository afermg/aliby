...
def test_dispatch_image_types():
    ...
    assert dispatch_image({"path": "/path.zarr", "key": "1"}) is ImageZarrArray
    assert dispatch_image("*.tif") is ImageList
    assert dispatch_image("/path/to/data.zarr") is ImageZarr
...
def test_image_zarr_array(data_dir, dataset):
    ...
    try:
        ...
        if isinstance(e, np.exceptions.AxisError):
            pytest.skip(f"ImageZarrArray not compatible with Zarr groups: {e}")
        else:
            raise
