...
class ImageList(BaseLocalImage):
    ...
    def get_data_lazy(self) -> da.Array:
        ...
        This Image Class accounts for most common setups for image acquisition, a bunch
        of images in one or multiple (nested) folders, where individual images have 2D
        (YX), 3D (ZYX) or 4D (CZYX) dimensions. The file names or paths determine the
        other dimensions, or if it is missing it defaults to a value of 1.

        This assumes that files can contain YX, YXZ or YXZC dimensions, but
        it cannot skip the default order. It also allows for missing dimensions,
        and naming other dimensions at the file level, such as W (Well) or P (Plate)
        (these non-`TCZYX` dimensions are ignored).

        self.input_dimensions are the dimensions provided by individual images.
        self.capture_order are the groups generated by multiple files.
        self.dimorder_d are the capture_order dimensions and the number of items found per dimension.

        The union of these two may not be equal to TCZYX, case in which we pad the missing dimensions.
        """
        ...
        infile_indices = [0] * n_infile_dims
        for i, ind in enumerate(sorted_indices):
            arr[(*ind, *infile_indices)] = lazy_arrays[i]

        a = da.block(arr.tolist())

        # rechunk to the last 3 dimensions. Leave time and channel unchunked
        file_dims = [self.dimorder_d.get(k, 1) for k in "TCZ" if k not in infile_dims]
